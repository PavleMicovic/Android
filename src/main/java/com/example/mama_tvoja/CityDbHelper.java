...
public class CityDbHelper extends SQLiteOpenHelper {
    ...
    public void insert(Paket p){
        ...
        if(p1==null)
            ...
            remove(p.getLocation(), p.getDate_time());
            db.insert(TABLE_NAME, null, cv);
        }
        close();
    }
    ...
    public void remove(String city, String date){
        SQLiteDatabase db=this.getWritableDatabase();
        db.delete(TABLE_NAME, COLUMN_LOCATION+" = ? AND "+COLUMN_DATE_TIME+"= ?", new String[]{city, date});
    }
    public Paket contains1(String city){
        SQLiteDatabase db = this.getReadableDatabase();
        Paket p;
        Cursor cursor = db.rawQuery(" SELECT * FROM " + TABLE_NAME + " WHERE "
                + COLUMN_LOCATION + " = \"" + city + "\";", null, null);
